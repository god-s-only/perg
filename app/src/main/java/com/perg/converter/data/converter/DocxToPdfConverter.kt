...
class DocxToPdfConverter @Inject constructor(private val pdfWriter: PdfPageWriter) {
    suspend fun convert(context: Context, sourceUri: String, outputFile: File) {
        withContext(Dispatchers.IO) {
            ...
            if (lines.all { it.isBlank() }) throw IllegalArgumentException("EmptyDocument")
            pdfWriter.writeLines(lines, outputFile)
        }
    }
}
